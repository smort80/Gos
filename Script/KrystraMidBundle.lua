...
for i, minion in pairs(minionManager.objects) do
  if ValidTarget(minion) and minion ~= nil and GetTeam(minion) == MINION_ENEMY then
    ...
    if self.Config.farm.laneclear.elogic:Value() == 1 then
      if  self.Config.farm.laneclear.useE:Value()  and GetDistance(minion) <= 475 then
        self:CastE(minion)
      end
    elseif self.Config.farm.laneclear.elogic:Value() == 2 then
      local edmg = self:GetEDmg(minion)
      if  self.Config.farm.laneclear.useE:Value()  and GetDistance(minion) <= 475 and edmg >= minion.health then
        self:CastE(minion)
      end
    end
  end
...
for i, minion in pairs(minionManager.objects) do
  if ValidTarget(minion) and minion ~= nil and GetTeam(minion) == MINION_JUNGLE then
    if not GetCastName(myHero,_Q)  == "yasuoq3w"  then
    if  self.Config.farm.laneclear.useQ:Value()  and GetDistance(minion) <= 475 then
      self:CastQ(minion)
    end
  end
...
if Tia and IsReady(Tia) and ValidTarget(target, 400) then
  CastSpell(Tia)
end
end
...
if Rhyd and IsReady(Rhyd) and ValidTarget(target, 400) then
  CastSpell(Rhyd)
end
end
...
if Thyd and IsReady(Thyd) and ValidTarget(target, 400) then
  CastSpell(Thyd)
end
end
...
if Rand and IsReady(Rand) and EnemiesAround(myHero, 500 )>= self.Config.item.randuin.minrand:Value() then
  CastSpell(Rand)
end
end
